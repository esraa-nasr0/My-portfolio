...
export default function Skills() {
  return (
   <section
  id="skills"
  className="w-full min-h-screen flex flex-col items-center justify-center px-6 lg:px-12 scroll-mt-24"
>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 justify-items-center">
      {skills.map((skill, index) => (
        <motion.div
          ...
          className="flex flex-col items-center justify-center p-4 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 text-center w-full h-48 bg-transparent"
        >
          {/* Icon */}
          <div className="text-5xl mb-4">{skill.icon}</div>

          {/* Name */}
          <h2 className="text-base font-semibold mb-2 mt-4 bg-gradient-to-r from-fuchsia-300 via-purple-300 to-pink-300 bg-clip-text text-transparent">
            {skill.name}
          </h2>

          {/* Description */}
          <p className="text-gray-600 dark:text-gray-300 text-xs mt-2 font-semibold">
            {skill.desc}
          </p>
        </motion.div>
      ))}
